...
export type NumberMap<T = string> = { [key:number]: T };
...
if (ALPHABETUPPER.length !== 26) throw new Error("ALPHABETUPPER character set has incorrect length: "+ALPHABETUPPER.length);
if (ALPHABETLOWER.length !== 26) throw new Error("ALPHABETUPPER character set has incorrect length: "+ALPHABETLOWER.length);
if (ALPHABETLOWER !== ALPHABETUPPER.toLowerCase()) throw new Error("ALPHABETLOWER and ALPHABETUPPER do not match");
 ...
 */
export function sin(x: number) { return Math.sin(x * (Math.PI / 180)); }
/**
 * Calculates the cosine value of an angle in degrees.
 */
export function cos(x: number) { return Math.cos(x * (Math.PI / 180)); }
