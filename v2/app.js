...
export default class App extends util.Target {
  constructor() {
    ...
    Array.from(document.querySelectorAll("article section.nav")).forEach(
      (sect) => {
        const buttonArr = Array.from(
          sect.querySelectorAll(":scope > nav > button"),
        );
        const sectArr = Array.from(
          sect.querySelectorAll(":scope > section > section"),
        );
        buttonArr.forEach((button, i) => {
          button.addEventListener("click", (e) => {
            ...
            sectArr.forEach((sect) => {
              if (sect.id == button.id + "-content") sect.classList.add("this");
    ...
    this.addHandler("setup", () => {
      ...
      if (canvas && canvas.getContext) {
        ...
        for (let i = 0; i < 450; i++)
          stars.push({
            ...
            d: 360 * Math.random(),
            l: util.lerp(5, 20, Math.random()),
            t: util.lerp(1000, 3000, Math.random()),
            to: Math.random(),
            r: Math.random(),
          });
        const update = (delta) => {
          ...
          stars.forEach((star) => {
            ...
            x += ox;
            y += oy;
            const c = new util.Color(
              star.r < 0.5
                ? [0, 128, 255, star.r / 0.5]
                : [
                    255 * ((star.r - 0.5) / 0.5),
                    util.lerp(128, 255, (star.r - 0.5) / 0.5),
                    255,
                    0.5,
                  ],
            );
        ...
        const animation = () => {
          ...
          const elems = [
            ...
            },
            {
              ...
              i: newline ? 1 : 0,
              type: "content",
              text: "Fellow developer, designer",
              size: "1em",
            ...
            },
            {
              t:
                ...
                tPause +
                (autocomplete ? 0 : tTag + tPause) +
                (autocomplete ? tTag : tTag + tPause + tContent2 + tPause),
              l: tTag,
              at: autocomplete ? 4 : 5,
          ...
          const cursor = autocomplete
            ? [
                ...
                {
                  t: tTag + tPause + tContent1 + tPause + tTag + tPause,
                  at: 4,
                },
              ]
            : [{ t: 0, at: 1000 }];
          ...
          const update = (delta) => {
            ...
            elems.forEach((data) => {
              ...
              if (!data.elem) {
                ...
                if (data.type == "content")
                  data.elem.style.fontSize = data.size;
                if (data.init) data.init(data.elem);
                let at = data.at;
                const children = Array.from(eTitleCard.children).filter(
                  (elem) =>
                    !(elem instanceof HTMLBRElement) &&
                    !elem.classList.contains("cursor"),
                );
                if (at < 0 || at >= children.length) {
                  eTitleCard.appendChild(data.elem);
                  if (data.br)
                    eTitleCard.appendChild(document.createElement("br"));
                } else {
                  const before = children[at];
                  eTitleCard.insertBefore(data.elem, before);
                  if (data.br)
                    eTitleCard.insertBefore(
              ...
              const text = String(
                data.type == "tag"
                  ? data.wraps[0] + data.text + data.wraps[1]
                  : data.text,
              );
              ...
              if (data.type == "tag") {
                data.elem.innerHTML = "";
                data.elem.appendChild(
                  document.createTextNode(
                    text.substring(0, Math.min(i, data.wraps[0].length)),
                  ),
                );
                ...
                data.elem.lastChild.textContent = text.substring(
                  ...
                  Math.min(
                    ...
                    Math.max(data.wraps[0].length, i),
                  ),
                );
                data.elem.appendChild(
                  document.createTextNode(
            ...
            if (at < 0 || at >= children.length) {
              if (children.at(-1) && children.at(-1).nextSibling) {
                ...
                eCursor.style.fontSize = children
                  .at(-1)
                  .classList.contains("tag")
                  ? "1em"
                  : elems.find((data) => data.elem == children.at(-1)).size;
              } else {
                eTitleCard.appendChild(eCursor);
              ...
              eCursor.style.fontSize = children[at].classList.contains("tag")
                ? "1em"
                : elems.find((data) => data.elem == children[at]).size;
            }
            if (t <= tStop) return;
            progressing = false;
      ...
      const p = 0.1;
      Array.from(document.querySelectorAll("body > article")).forEach(
        (elem) => {
          const update = (delta) => {
            let forceShow = false;
            Array.from(elem.querySelectorAll("section.list.normal")).forEach(
              (sect) => {
                // forceShow = true;
                ...
                aArr.forEach((a) => {
                  ...
                  if (
                    ...
                    sect.style.setProperty(
                      "--y",
                      r.top + r.height / 2 < window.innerHeight / 2 ? -1 : +1,
                    );
                    if (a._idIn) {
            ...
            Array.from(
              elem.querySelectorAll(
                "section.list.big-items, section.list.small-items",
              ),
            ).forEach((sect) => {
            ...
            Array.from(elem.querySelectorAll("section.timeline")).forEach(
              (sect) => {
