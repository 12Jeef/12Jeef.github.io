...
export default class PatternWorkerScript extends WorkerScript {
    ...
    clampX(x, y, i) {
        ...
        if (this.larvaCache.x[x])
            return this.larvaCache.x[x];
        let x2 = x;
    ...
    clampY(x, y, i) {
        ...
        if (this.larvaCache.y[x])
            if (this.larvaCache.y[x][y])
                return this.larvaCache.y[x][y];
        let y2 = y;
    ...
    setup() {
        ...
        let callbackfs = {
            ...
            "1": (v, idx, x, y, i) => {
                if (i > 1) return;
                this.space[idx] = (Math.sqrt((x-this.width/2) ** 2 + (y-this.height/2) ** 2) < 2.5) * 1;
            },
            "2": (v, idx, x, y, i) => {
                if (i !== 2) return;
            ...
            "3": (v, idx, x, y, i) => {
                if (this.inLarva(x, y)) {
                    ...
                    this.larvaHeightSegmentRange[0] = Math.min(this.larvaHeightSegmentRange[0], x);
                    this.larvaHeightSegmentRange[1] = Math.max(this.larvaHeightSegmentRange[1], x);
                    if (!(x in this.larvaHeightSegments))
                        this.larvaHeightSegments[x] = [y, y];
                    this.larvaHeightSegments[x][0] = Math.min(this.larvaHeightSegments[x][0], y);
                    this.larvaHeightSegments[x][1] = Math.max(this.larvaHeightSegments[x][1], y);
                }
                if (i === 0) {
                    this.space[idx] = util.lerp(-1, +1, Math.random()) * this.inLarva(x, y);
                    return;
                }
                if (i === 2) {
    ...
    applyChannel(i, v) { return v*v * [64, 128, 64][i]; }
    ...
    inLarva(x, y) {
        return ((x-this.width/2) / (this.width/4))**2 + ((y-this.height/2) / (this.height/4))**2 < 1;
    }
