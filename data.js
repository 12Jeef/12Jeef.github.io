...
let allowedNormal = " 0123456789)!@#$%^&*(abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ`~-_=+[{]}\\|;:'\",<.>/?".split("");
let allowedWeird = [
    "🔥", "💀", "😱", "🤓", "🥵",
    "🥶", "😭", "😢", "😡", "🤬",
    "😩", "😎", "🤡", "👍", "👎",
...
export class Buffer extends util.Target {
    ...
    read(x, l) {
        ...
        if (x >= this.#dat.length) throw `X out of bounds (X=${x}, L=${l}, l=${this.#dat.length})`;
        if (l < 0) throw `L out of bounds (L=${l})`;
        if (l > 0) {
            if (x+l-1 < 0) throw `X+L out of bounds (X=${x}, L=${l}, l=${this.#dat.length})`;
            if (x+l-1 >= this.#dat.length) throw `X+L out of bounds (X=${x}, L=${l}, l=${this.#dat.length})`;
        }
        let v = 0, b = 1;
        for (let i = 0; i < l; i++) {
            v += b * this.#dat[x+i];
    ...
    write(x, l, v, push=true) {
        ...
        if (push) {
            ...
            this.#pad();
        } else if (x >= this.#dat.length) throw `X out of bounds (X=${x}, L=${l}, l=${this.#dat.length})`;
        if (l < 0) throw `L out of bounds (L=${l})`;
        if (l > 0) {
            if (x+l-1 < 0) throw `X+L out of bounds (X=${x}, L=${l}, l=${this.#dat.length})`;
            if (push) {
                ...
                this.#pad();
            } else if (x+l-1 >= this.#dat.length) throw `X+L out of bounds (X=${x}, L=${l}, l=${this.#dat.length})`;
        }
...
export class Match extends util.Target {
    ...
    constructor(id, robot) {
        ...
        this.#pos = new V();
        this.pos.addLinkedHandler(this, "change", (c, f, t) => this.change("pos."+c, f, t));
        this.#preloaded = null;
        ...
        this.teleopFrames.addLinkedHandler(this, "change", (c, f, t) => this.change("teleopFrames."+c, f, t));
    ...
    hasRobot() { return !!this.robot; }
    get robotTeam() { return this.#robotTeam; }
    set robotTeam(v) {
    ...
    hasRobotTeam() { return this.robotTeam != null; }
    ...
    hasTeleopTime() { return this.teleopTime != null; }
    get teleopFrames() { return this.#teleopFrames; }
    set teleopFrames(v) {
        if (v instanceof Match.Frames) return this.teleopFrames.frames = v.frames;
        this.teleopFrames.frames = v;
    }

    get endgameTrap() { return this.#endgameTrap; }
    set endgameTrap(v) {
        v = !!v;
    ...
    get endgameHarmony() { return this.#endgameHarmony; }
    set endgameHarmony(v) {
        v = !!v;
        if (this.endgameHarmony == v) return;
        this.change("endgameHarmony", this.endgameHarmony, this.#endgameHarmony=v);
        console.log("endgameHarmony = "+JSON.stringify(this.endgameHarmony));
    }

    get finishTime() { return this.#finishTime; }
    set finishTime(v) {
        v = (v == null) ? null : util.ensure(v, "num");
    ...
    hasFinishTime() { return this.finishTime != null; }
    ...
    static toBufferStr(data) {
        ...
        }
        {
            ...
            // writing
            buff
                .write(i, 1, endgameTrap)
                .write(i+1, 1, endgameHarmony)
            ;
            i += 1+1;
        }
        {
        ...
        return util.Target.resultingForEach(frames, frame => {
            ...
            if (this.#frames.length == 0) this.#frames.push(frame);
                ...
                if (frame.ts < this.#frames.at(0).ts) this.#frames.unshift(frame);
                    ...
                    let l = 0, r = this.#frames.length-2;
                    while (l <= r) {
            ...
            console.log("add frame<"+this.name+"> ("+frame.type+" = "+JSON.stringify(frame.state)+" @ "+frame.ts+")");
            return frame;
        });
    }
    rem(...frames) {
        return util.Target.resultingForEach(frames, frame => {
            ...
            console.log("rem frame<"+this.name+"> ("+frame.type+" = "+JSON.stringify(frame.state)+" @ "+frame.ts+")");
            return frame;
        });
    }
